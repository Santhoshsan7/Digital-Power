    ...
    useEffect(() => {
        const fetchLowStock = async () => {
            try {
                ...
                if (data.success) {
                    const filtered = data.products.filter(p => p.stock < 5);
                    setLowStockItems(filtered.map(p => ({
                        name: p.name,
                        brand: p.brand,
                        stock: p.stock,
                        status: p.stock < 3 ? "Critical" : "Low",
                        trend: p.stock === 0 ? "Out of Stock" : "Depleting"
                    })));
                }
            } catch (error) {
        ...
        <div className="space-y-8 animate-fadeIn w-full px-4 sm:px-8 pb-12">
            ...
            <div className="bg-white rounded-[2rem] shadow-xl border border-red-50 overflow-hidden">
                ...
                <div className="overflow-x-auto">
                    {loading ? (
                        <div className="p-10 text-center animate-pulse text-gray-400 font-bold uppercase tracking-widest text-xs">Scanning Inventory...</div>
                    ) : (
                        <table className="w-full text-left">
                            ...
                            <tbody className="divide-y divide-gray-100">
                                {lowStockItems.length > 0 ? lowStockItems.map((item, i) => (
                                    ...
                                    <tr>
                                        <td colSpan="5" className="px-10 py-10 text-center text-green-600 font-black uppercase text-xs tracking-widest">
                                            ✅ All Items are in Safe Stock Levels
                                        </td>
                                    </tr>
                                )}
                            </tbody>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-8 pt-4">
                 <div className="bg-white p-8 rounded-3xl border border-gray-100 flex gap-5 items-start">
                    ...
                    <div>
                        ...
                        <p className="text-gray-500 text-sm font-medium leading-relaxed">System recommends ordering <span className="text-brand-navy font-bold">buffer stock</span> for Critical items to avoid procurement delays.</p>
                    </div>
                 </div>
                 <div className="bg-white p-8 rounded-3xl border border-gray-100 flex gap-5 items-start">
                    ...
                    <div>
                        <h4 className="font-black text-brand-navy mb-1 uppercase text-xs tracking-widest">Inventory Health</h4>
                        <p className="text-gray-500 text-sm font-medium leading-relaxed">Maintain a minimum of <span className="text-red-600 font-bold">5 units</span> for all high-value inverters to ensure zero customer downtime.</p>
                    </div>
                 </div>
            </div>
        </div>
