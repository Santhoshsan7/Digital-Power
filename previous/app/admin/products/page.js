...
function ProductsManagementContent() {
    ...
    // Initial display products filtered by category if present
    const initialProducts = catFilter 
        ? allProducts.filter(p => p.category?.toLowerCase() === catFilter.toLowerCase() || p.category?.toLowerCase() === catFilter.replace(/s$/, '').toLowerCase())
        : allProducts;

    const [products, setProducts] = useState(initialProducts);
    const [searchTerm, setSearchTerm] = useState("");

    // Sync state if category changes via URL
    useEffect(() => {
        ...
        <div className="space-y-8 animate-fadeIn w-full px-4 sm:px-8 pb-12">
            ...
            <div className="bg-white p-4 rounded-2xl shadow-sm border border-gray-100 flex flex-col md:flex-row gap-4 items-center">
                ...
                <div className="flex gap-2">
                    <select className="bg-gray-50 border border-gray-100 text-xs font-bold p-2.5 rounded-xl text-gray-600 outline-none">
                                ...
                                <tr key={p.id} className="hover:bg-gray-50/50 transition-all group">
                                    <td className="px-8 py-6">
                                        <div className="flex items-center gap-4">
                                            <div className="w-12 h-12 bg-gray-50 rounded-xl overflow-hidden border border-gray-100 group-hover:border-brand-orange transition-colors shrink-0">
                                                <img src={p.image} alt={p.name} className="w-full h-full object-contain p-1" />
                                            </div>
                                            <div className="flex flex-col min-w-0">
                                                <span className="text-sm font-black text-brand-navy truncate">{p.name || "Unnamed Product"}</span>
